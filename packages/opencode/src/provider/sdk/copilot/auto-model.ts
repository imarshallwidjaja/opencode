...
const AUTO_MODEL_ALLOWLIST_ENV = "COPILOT_AUTO_MODEL_ALLOWLIST"
...
function getAutoModelAllowlist(): Set<string> | undefined {
  const value = process.env[AUTO_MODEL_ALLOWLIST_ENV]
  if (value === undefined) return undefined
  return new Set(value.split(",").map((item) => item.trim()).filter(Boolean))
}

function getRoutingCandidates(session: CopilotAutoModelSession, executableModelIds?: string[]): string[] {
  const executable = new Set(executableModelIds ?? session.availableModels)
  const allowlist = getAutoModelAllowlist()
  return session.availableModels.filter((modelId) => executable.has(modelId) && (!allowlist || allowlist.has(modelId)))
}

function getNoCandidateMessage(): string {
  if (process.env[AUTO_MODEL_ALLOWLIST_ENV] !== undefined) {
    return `No Copilot auto models are available after applying ${AUTO_MODEL_ALLOWLIST_ENV}`
  }
  return "No locally executable Copilot auto models are available"
}
...
export class CopilotAutoLanguageModel implements LanguageModelV3 {
  ...
  private async resolveModel(callOptions: LanguageModelV3CallOptions): Promise<LanguageModelV3> {
    ...
    const availableModels = getRoutingCandidates(session, this.options.executableModelIds)
    if (availableModels.length === 0) throw new Error(getNoCandidateMessage())
    ...
    if (
      ...
      this.lastModelId &&
      availableModels.includes(this.lastModelId)
    ) {
    ...
    const response = await this.options.fetch(url, {
      ...
      body: JSON.stringify({
        ...
        previous_model:
          this.lastModelId && availableModels.includes(this.lastModelId) ? this.lastModelId : undefined,
    ...
    if (!availableModels.includes(selectedModelId)) {
      throw new Error(`Failed to route auto model session: selected unavailable model ${selectedModelId}`)
    }
