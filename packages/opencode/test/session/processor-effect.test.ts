...
let earlyResolvedModelGate: Deferred.Deferred<void> | undefined

const earlyResolvedModelLLM = Layer.succeed(
  LLM.Service,
  LLM.Service.of({
    stream: () =>
      Stream.concat(
        ...
        Stream.fromEffect(
          Deferred.await(earlyResolvedModelGate!).pipe(
            Effect.as({
              type: "finish",
              finishReason: "stop",
              usage: {
...
const itResolvedModelEarly = testEffect(
  SessionProcessor.layer.pipe(
    Layer.provide(summary),
    Layer.provideMerge(depsWithoutLLM),
    Layer.provideMerge(earlyResolvedModelLLM),
  ),
)
...
itResolvedModelEarly.effect("session.processor effect tests persist resolved model id before stream completion", () =>
  provideTmpdirInstance(
    (dir) =>
      Effect.gen(function* () {
        const gate = yield* Deferred.make<void>()
        earlyResolvedModelGate = gate
        const processors = yield* SessionProcessor.Service
        const session = yield* Session.Service
        ...
        yield* Effect.promise(async () => {
          ...
          while (Date.now() < stop) {
            const messages = await Effect.runPromise(session.messages({ sessionID: chat.id }))
            const updated = messages.find((item) => item.info.id === msg.id)
            if (updated?.info.role === "assistant" && updated.info.modelID === ModelID.make("resolved-early-model")) return
            await Bun.sleep(10)
          }
          throw new Error("timed out waiting for resolved model sync")
        })
