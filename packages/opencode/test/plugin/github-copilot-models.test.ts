...
test("includes every final Copilot model in Auto executable model ids", async () => {
  globalThis.fetch = mock(() =>
    Promise.resolve(
      new Response(
        JSON.stringify({
          data: [
            {
              ...
              version: "claude-sonnet-4.6-2026-01-01",
  ...
  expect(models["claude-sonnet-4.6"].api.npm).toBe("@ai-sdk/anthropic")
  expect(models.auto.options.executableModelIds).toEqual(["claude-sonnet-4.6", "gpt-5.4"])
})
